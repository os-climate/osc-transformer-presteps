...
if __name__ == "__main__":
    ...
    while not real_file:
        file_name = input(
            f"What is the file name you want to extract? (Folder is {input_folder})"
        )
        input_file_path_main = input_folder / file_name
        real_file = input_file_path_main.is_file()
        if not real_file:
            print(
                "The input file can't be found. Please recheck and enter a valid file."
            )
    ...
    if real_file and input_file_path_main is not None:
        ...
        server_live_check = requests.get(
            f"{url}/liveness", proxies={"http": "", "https": ""}
        )
        if server_live_check.status_code == 200:
            file = {"file": open(str(input_file_path_main), "rb")}
            response = requests.post(
                f"{url}/extract", files=file, proxies={"http": "", "https": ""}
            )
