...
class TestKPICurator:
    ...
    def test_run_kpi_curator(self, mock_logger, mock_curate, mock_curator_data, mock_train_val_dfs):
        ...
        expected_train_output = os.path.join(mock_curator_data["output_folder"], f"train_kpi_data_{today_date}.xlsx")
        expected_val_output = os.path.join(mock_curator_data["output_folder"], f"val_kpi_data_{today_date}.xlsx")
        ...
        mock_logger.info.assert_any_call(f"Train data saved to: {expected_train_output}")
        mock_logger.info.assert_any_call(f"Validation data saved to: {expected_val_output}")
    ...
    @patch('src.osc_transformer_presteps.kpi_curator_main.curate', side_effect=Exception("Test error"))
    @patch('src.osc_transformer_presteps.kpi_curator_main._logger')
