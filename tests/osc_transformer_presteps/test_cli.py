...
def test_extraction_command(runner):
    ...
    assert "If you want to run local extraction of text from files to json then this is the subcommand to use." in result.output


def test_curation_command(runner):
    result = runner.invoke(app, ["curation"])
    assert result.exit_code == 0
    assert "If you want to run local creation of dataset of json files then this is the subcommand to use." in result.output
